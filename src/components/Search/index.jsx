...
export default function Search({ senders, filterdMessages = [], setFilterdMessages }) {

    const onChange = (e) => {
        const input = e.target.value;
        let newFilter = senders.filter(sender => sender.sender.toLowerCase().includes(input.toLowerCase()));
        setFilterdMessages(newFilter);
    };
    ...
    return (
        <form className={styles.search} onSubmit={(e) => e.preventDefault()}>
            <label>
                ...
                <button type="reset" className={styles.closebtn} onClick={() => setFilterdMessages(senders)}>
